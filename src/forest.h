...
class Forest
{
 public:
  ...
  void sampleNextBase() {
      // TODO: should the call to model().recombination_rate() not include the current sequence index,
      //       so that the actual recombination rate can change?
    ...
    if (length == -1) {
      ...
      set_next_base(current_base_ + length);
    }
  }
  ...
  void resampleNextBase() {
      ...
      if (next_base_ == model().getCurrentSequencePosition()) {
